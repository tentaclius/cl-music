...
(def g (gen-list [:sus4 :sus9 :7sus4 :7]))

(defun rn (chance lst)
  (remove-if (λ(e) (= 0 (random chance)))
             lst))
...
(defpattern ssin
  (play-note 'ssin
             :attr [:out (abus :ssin) :dur 1/4 :amp 0.2 :r 1/2 :a 0.01 :q 1 :depth 100]
             :note-fn (λ(n) [:freq (midicps (+ 50 (sc *minor* n)))]))
  (λ(i)
    (per-beat i
              (seql (rn 3 (chord :3)) (rn 2 (chord :3 7)))
              (seql (rn 3 (chord :7 3)) (rn 2 (chord :7 (+ 3 7))))
              (seql (rn 3 (chord (funcall g) 5)) (rn 2 (chord (funcall g) (+ 5 7))))
              (seql (rn 3 (chord :3 4)) (rn 2 (chord :3 (+ 4 7))))
              ))
  1)
...
(defpattern drums
  ...
  (λ(i)
    (let ((o nil)
          (h ['hh :dur 0.040])
          (b ['bd]))
      (sim (seq o h o (per-beat i h h h (seq h h)))
           (per-beat i
             (seq b b))))))
...
(defpattern bass
  (play-note 'fm-bass
             :release t
             :attr [:q 1 :depth 100 :amp 1/8 :a 0.4 :d 1]
             :note-fn (λ(n) [:freq (midicps (+ 50 -12 (sc *minor* n)))]))
  (λ(i)
    (sim
      ;(seq [4 :freq0 20 :slide 1])
      (per-beat i
              (seq 0 2 3 4)
              (seq 0 2 6 4))
      )
...
(defpattern ssin
  ...
  (λ(i)
    (per-beat i
              ...
              (seql (rn 4 (chord :7 3)) (rn 4 (chord :7 (+ 3 7))))
              (seql (rn 4 (chord (funcall g) 5)) (rn 2 (chord (funcall g) (+ 5 7))))
              (seql (rn 4 (chord :3 4)) (rn 4 (chord :3 (+ 4 7))))
              ))
  1)
...
(defpattern ssn
  (play-note 'ssin
             :release t
             :attr [:dur 1/8 :r 1 :amp 1/3]
             :note-fn (λ(n) [:freq (midicps (+ 60 n))]))
  (λ(i)
    (seq
      (siml (chord :3 5 0 *minor*))
      (siml (chord :3 3 0 *minor*))
      (siml (chord :3 0 0 *minor*))
      (siml (chord :3 2 0 *minor*))
      ))
  4)
(ssn :start 1 2)
...
(defsynth bang
  ((freq 70) (freq0 70) (slide 0) (amp 0.9)
              (out 0) (gate 1)
              (noise 0) (noise-lpf 11000)
              (a 0.004) (d 0.4))
  (let ((fq (x-line.kr freq0 freq slide)))
    (-<> (sin-osc.ar fq)
         (+ (* noise (lpf.ar (white-noise.ar) noise-lpf)))
         (* amp (env-gen.kr (perc a d) :gate gate :act :free))
         pan2.ar (out.ar out <>))))

(defsynth zap((freq 110) (freq0 440) (slide 0) (amp 0.3)
              (out 0) (gate 1)
              (a 0.1) (d 0.2) (r 0.2))
  (let ((fq (x-line.kr freq0 freq slide)))
    (-<> (saw.ar fq)
...
(synth 'bang :noise 0.1 :noise-lpf 400)
(synth 'bang :freq 35)
(synth 'bang :freq0 550 :slide 0.03 :freq 20)
(synth 'bang :freq0 11000 :slide 0.05 :freq 20)

(synth 'zap :freq 100 :a 0.001 :d 0.051 :r 0)
...
(defpattern drums
  ...
  (λ(i)
    (let ((o nil)
          (b0 ['bang :freq 100])
          (b1 ['bang :noise 0.1 :noise-lpf 400])
          (b2 ['bang :freq 35])
          (b3 ['bang :freq0 220 :slide 0.03 :freq 20])
          (b4 ['bang :freq0 600 :slide 0.06 :freq 20])
          (b5 ['bang :freq 40 :noise 0.9 :noise-lpf 400 :amp 1])
          (z0 ['zap :freq 100 :a 0.001 :d 0.05 :r 0])
          (z1 ['zap :freq 7000 :a 0.001 :d 0.01 :r 0.4]))
      (sim 
           (per-beat i
             (seq b1 o  z0 b1 o  b3 o  b3)
             (seq b1 b5 b3 b1 o  b3 o  b4)
             (seq b5 b0 b3 o  b2 b3 o  z0)
             (seq z0 b0 b3 z1 o  z1 b0 b3)
             )))))
